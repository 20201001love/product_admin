...
- **操作**: 使用 dhtmlx-gantt 实现任务派工的甘特图可视化功能
- **原因**: 提供直观的任务排程可视化，方便生产管理人员查看和分析机台派工情况
- **变更文件**:
  - `src/api/pps/gantt.js`: 甘特图数据接口
  - `src/views/pps/gantt/index.vue`: 甘特图页面组件
  - `package.json`: 添加 dhtmlx-gantt 依赖
- **功能特性**:
  1. **按机台分组显示**: 以机台为分组，树形结构展示派工任务
  2. **时间缩放**: 支持日视图和时视图两种时间尺度
  3. **多维度筛选**: 支持按车间、机台、任务状态、日期范围筛选
  4. **任务详情**: 点击任务条块弹出详情对话框，显示批次、工序、计划时间、预计时长等信息
  5. **状态可视化**: 不同任务状态使用不同颜色标识（已调度、运行中、已完成、暂停、已取消）
  6. **模拟数据**: 开发阶段使用模拟数据，包含5台机器和10个任务示例
- **技术实现**:
  - dhtmlx-gantt 9.1.1
  - 自定义列配置（机台、开始时间、时长）
  - 树形结构展示（project + task）
...
- **后端接口要求**:
  - 接口路径: `/pps/gantt/data`
  - 请求参数: `orgUnit`, `machineId`, `status`, `startDate`, `endDate`
  - 响应格式:
    ```json
    {
      "code": 200,
...
- **使用说明**:
  1. 在路由中添加甘特图页面路由
  2. 当后端接口完成后，将 `getMockGanttData` 替换为 `getGanttData`
  3. 可根据实际需求调整时间尺度、列配置等
